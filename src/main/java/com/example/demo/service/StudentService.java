...
public class StudentService
{
    ...
    public StudentEntity signup(StudentRegisterDto input)
    {
        ...
        StudentEntity studentEntity=new StudentEntity(input.getUsername(), input.getName(), encodedPassword,input.getMail(),input.getTenthBoard(), input.getTenthMarks(), input.getTwelfthBoard(), input.getTwelfthMarks(), input.getTwelfthStream(), input.getAnyCompExam(), input.getCompExamRank());
    ...
    {
        //Optional<StudentEntity> optionalStudent=studentRepository.findByMail(input.getUsername());
        StudentEntity studentEntity=studentRepository.findByUsername(input.getUsername());
    ...
    public boolean updateStudentDetails(String username,StudentEntity updatedStudent)
    {
        ...
        if(existingStudent!=null&&existingStudent.isEmailVerified())
        {
            ...
            existingStudent.setTenthBoard(updatedStudent.getTenthBoard());
            existingStudent.setTenthMarks(updatedStudent.getTenthMarks());
            existingStudent.setTwelfthBoard(updatedStudent.getTwelfthBoard());
            existingStudent.setTwelfthMarks(updatedStudent.getTwelfthMarks());
            existingStudent.setTwelfthStream(updatedStudent.getTwelfthStream());
            existingStudent.setAnyCompExam(updatedStudent.getAnyCompExam());
            existingStudent.setCompExamRank(updatedStudent.getCompExamRank());
